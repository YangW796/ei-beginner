...
ROBOT_URDF_PATH = "/home/wy/Document/eibeginner/model/ur5-bullet/UR5/ur_e_description/urdf/ur5e_with_camera.urdf"
...
class UR5Sim():
    ...
    def get_camera_image(self, width=640, height=480):
        """ 获取摄像头图像（RGB和深度） """
        view_matrix = pybullet.computeViewMatrixFromYawPitchRoll(cameraTargetPosition=[0, 0, 0], distance=1.2, yaw=90, pitch=-30, roll=0,  upAxisIndex=2    )
        projection_matrix = pybullet.computeProjectionMatrixFOV(fov=60, aspect=width/height, nearVal=0.1, farVal=100)
          # 获取图像数据
        image = pybullet.getCameraImage(width, height, view_matrix, projection_matrix, shadow=True, lightDirection=[1, 1, 1])
        
        rgb_array = np.reshape(image[2], (height, width, 4))  # 获取 RGB 图像
        depth_array = np.reshape(image[3], (height, width))  # 获取深度图像
